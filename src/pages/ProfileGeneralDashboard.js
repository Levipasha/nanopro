/**
 * ProfileGeneralDashboard.js
 * Restructured general dashboard copied from artist dashboard.
...
import ProfileGeneralProfiles from './ProfileGeneralProfiles';
import ProfileGeneralPlatforms from './ProfileGeneralPlatforms';
import ProfileGeneralDesign from './ProfileGeneralDesign';
import ProfileGeneralLinkArt from './ProfileGeneralLinkArt';
...
export default function ProfileGeneralDashboard(props) {
  const {
  ...
  const handleSubmitUsername = async () => {
    ...
    setPopupError(null);
    try {
      if (handleCreateProfileWithUsername) {
        await handleCreateProfileWithUsername(localUsername);
      }
    } catch (err) {
      setPopupError(err.message || 'Failed to create profile. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    if (iframeRef.current && iframeRef.current.contentWindow) {
      try {
        const cleanData = JSON.parse(JSON.stringify({
          ...
          title: heroUpdates.experience !== undefined ? heroUpdates.experience : (artist?.title || ''),
  ...
  const renderEditorHeader = (title, fieldName, actionBtn = null) => {
    ...
    const handleToggle = () => {
      ...
      // Save to database
      handleUpdateHeroField(fieldName, newVal);
    };
  ...
  const renderPlatformsEditor = () => (
    <div>
      ...
      <ProfileGeneralPlatforms {...props} hidePreview={true} />
    </div>
  );
  ...
  const renderDesignEditor = () => (
    <div>
      ...
      <ProfileGeneralDesign {...props} hidePreview={true} />
    </div>
  );
  ...
  const renderWhatIDoEditor = () => (
    <div>
      ...
      <ProfileGeneralLinkArt {...props} activeTab="what-i-do" hidePreview={true} />
    </div>
  );
  ...
  if (isMobileViewport) {
    return (
      <div className={`dash-root dash-theme-${dashTheme} dash-font-${dashFont} dash-mode-general`} style={{
        position: 'relative',
        display: 'flex',
        flexDirection: 'column',
        height: '100vh',
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            {myArtists && myArtists[0] && (() => {
              const profileUrl = `${frontendBase}/link/${myArtists[0].artistId}`;
              return (
                <>
                  <button
                    onClick={() => {
            ...
            <div style={{
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '0.92rem', color: '#64748b', margin: 0, lineHeight: 1.5 }}>
                  Please choose a unique username to generate your Indian mock profile. You can edit everything later!
                </p>
              </div>
              ...
              >
                {submitting ? 'Creating Profile...' : 'Claim Username & Create Profile'}
              </button>
            </div>
          </div>
        , document.body)}
