...
import icons from './components/LinkIcons';
...
function App() {
  ...
  useEffect(() => {
      ...
      requestAnimationFrame(() => {
        ...
        ctx = gsap.context(() => {
          ...
          return () => {
            ...
            // Explicitly kill and revert pins
            if (trigger) {
              trigger.kill(true); // true = revert pinning/styling
            }
            scrub?.kill();
            seamlessLoop?.kill();
          };
    ...
    return () => {
      ...
      // Secondary sweep: Kill ANY remaining ScrollTriggers created by this component
      // and ensure they revert their pins immediately
      ScrollTrigger.getAll().forEach(t => {
        if (t.vars.trigger && typeof t.vars.trigger === 'string' && sections.includes(t.vars.trigger)) {
          t.kill(true);
        }
      });

      // Force a refresh to clean up the DOM markers
      ScrollTrigger.refresh();
    };
  }, [isGalleryRoute]);
          ...
          <main className="landing-page-container">
              ...
              <div className="navbar-inner">
                ...
                <div className={`navbar-links ${mobileMenuOpen ? 'mobile-open' : ''}`}>
                  <div className="nav-segmented">
                    ...
                    {!isLoggedIn && (
                      <Link to="/login" onClick={() => setMobileMenuOpen(false)} className="nav-highlight">Profiles</Link>
                    )}
                  </div>
                  {isLoggedIn && (
                    <div className="nav-avatar-wrap" ref={avatarMenuRef}>
                      <button
                      ...
                      {avatarMenuOpen && (
                        <div className="nav-avatar-dropdown">
                          <button
                            ...
                            onClick={() => { setAvatarMenuOpen(false); setMobileMenuOpen(false); navigate('/profile'); }}
                          >
                          ...
                          <button
                            className="nav-avatar-dropdown-item nav-avatar-logout"
            ...
            <section id="home" className="page-section" style={{ width: '100%', height: '100vh', position: 'relative', overflow: 'hidden' }}>
              ...
              <div className="stuck-grid" style={{ position: 'relative', zIndex: 1 }}>
                <div key="grid-item-0" className="grid-item special"><b>Nano Profiles</b></div>
                <div key="grid-item-1" className="grid-item">ID Card</div>
                <div key="grid-item-2" className="grid-item">NFC Technology</div>
                <div key="grid-item-3" className="grid-item">Data Security</div>
                <div key="grid-item-4" className="grid-item">Biometric Access</div>
                <div key="grid-item-5" className="grid-item">Digital Identity</div>
                <div key="grid-item-6" className="grid-item">Smart Authentication</div>
                <div key="grid-item-7" className="grid-item">Encryption</div>
                <div key="grid-item-8" className="grid-item">Secure Storage</div>
                <div key="grid-item-9" className="grid-item">Privacy Protection</div>
                <div key="grid-item-10" className="grid-item">Cloud Sync</div>
                <div key="grid-item-11" className="grid-item">Mobile Access</div>
              </div>
            </section>
              ...
              <div className="landing-carousel-wrapper">
                <div className="landing-carousel-inner" style={{ '--quantity': 10 }}>
                  {[
                    ...
                    <div key={i} className="landing-carousel-card" style={{ '--index': i, '--color-card': `${p.rgb[0]}, ${p.rgb[1]}, ${p.rgb[2]}` }}>
